...
layui.use(["okUtils", "okTab", "okLayer", "okContextMenu", "okHoliday"], function () {
	...
	var okTab = layui.okTab({
		// 菜单请求路径
		url: "/assets/uadmin/data/navs.json",
		// 允许同时选项卡的个数
		openTabNum: 30,
	...
	$("body").on("click", "#okSetting", function () {
		layer.open({
			...
			time: 200000, //2秒后自动关闭
			anim: -1,
			content: "/center.php?s=system/setting.html"
		});
	});
	...
	$("#alertSkin").click(function () {
		okLayer.open("皮肤动画", "/center/system/alertSkin.html", "50%", "45%", function (layero) {
		}, function () {
		});
	});
	...
	$("#lock").click(function () {
		okLayer.confirm("确定要设置锁屏吗？", function (index) {
			layer.close(index);
